// `:usage-source-psi` — the Kotlin **parser** behind the usage cleaner, kept off the server's
// runtime classpath.
//
// `PlaygroundSourceCleaner` used to answer every structural question with regex, because the Kotlin
// frontend is deliberately not a server dependency (see `server/build.gradle.kts`'s sidecar note).
