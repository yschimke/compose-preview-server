...
allowlist with `SERVE_UI_BUILDER_CATALOGS`. `remote-m3` is the reviewed Wear-widget subset (Small and Large
...
on a shelf an author switches on from the editor's settings. A pack's component record is read from
...
[catalog MCP guide](../../docs/design/CATALOG_MCP.md).
