 ...
 *
 * The baked lane replays numbers the offline run computed with pixelmatch; every other reference is
...
export function statusFor(reference: string, shortLabel: string): string {
    ...
    if (reference === BAKED)
        return "showing the build-time pixel diffs against the baked PNG";
    return (
