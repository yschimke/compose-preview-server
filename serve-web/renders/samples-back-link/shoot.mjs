// Screenshot a SAMPLES page's component drawer, with and without the derived back-link — the
// "Explains" directory naming the kit component this sample is a call site for.
//
//   node shoot.mjs
//
...
const file = before
    ? (() => {
          ...
          const out = path.join(dir, "serve-viewer-samples.html");
          fs.writeFileSync(out, fromGit("preview-harness/fixtures/pages/serve-viewer-samples.html"));
          return out;
      })()
    : path.join(pages, "serve-viewer-samples.html");
...
await tab.screenshot({
    path: path.join(here, "drawer.png"),
    clip: {
...
console.log(`drawer: ${Math.round(box.width)}×${Math.round(box.height)}`);
await browser.close();
