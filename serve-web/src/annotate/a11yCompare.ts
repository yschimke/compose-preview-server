...
export function a11yDifferences(
    ...
    // `matchAnnotationItems` retains a one-sided list unchanged. Give that list stable ordinals
    // here: otherwise every item occupies the `undefined` map key and only its last finding wins.
    const byOrdinal = (items: AnnotationItem[]) =>
        new Map(
            items.map((item, index) => [
                item.comparisonOrdinal ?? index + 1,
                item,
            ]),
        );
    const reference = byOrdinal(matched.reference);
    const actual = byOrdinal(matched.actual);
