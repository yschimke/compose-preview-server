...
/** `.annotations` wraps its items, unlike the inline imported-spec payload. */
function annotationItemsOf(payload: unknown): unknown {
    if (
        payload &&
...
export class SpecCompare extends ControllerElement {
    ...
    /** Endpoint promises keyed by URL, so the two sides of a pair each remain reusable. */
    private dataPromises = new Map<string, Promise<unknown>>();
    ...
    private referenceAnnotations(): Promise<unknown> {
        if (!this.sourceIsSpec)
            return this.fetchData(
                dataUrlFor(this.reference(), "annotations"),
            ).then(annotationItemsOf);
    ...
    private fetchData(url: string | null): Promise<unknown> {
        ...
        const cached = this.dataPromises.get(url);
        if (cached) return cached;
        const request = fetch(url, { credentials: "same-origin" })
            .then((response) => (response.ok ? response.json() : null))
            .catch(() => null);
        this.dataPromises.set(url, request);
        return request;
    }
