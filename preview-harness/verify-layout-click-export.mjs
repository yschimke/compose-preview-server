...
const output = fileURLToPath(new URL("../docs/design/evidence/ui-builder-layout-clicks/", import.meta.url));
...
const source = await readFile(new URL("../docs/design/fixtures/ui-builder/clickable-state-layout.kt.txt", import.meta.url), "utf8");
...
const catalog = (await mcp("ui_builder_list_catalogs", {})).catalogs.find(c => c.systemId === "m3-catalog");
assert(catalog);
sample.id = "layout-clicks-" + Date.now();
sample.title = "Clickable state layout";
...
assert(artifact.content.endsWith(source));
...
try {
    ...
    await page.goto(`${origin}/ui-builder/m3-catalog/${sample.id}?token=${encodeURIComponent(token)}&node=choice`);
    ...
    const verification = { designId: sample.id, revision: 0, catalogPin: sample.catalogPin,
        ...
        sourceMatchesCompiledFixture: true, browserCodePaneCaptured: true, errors };
