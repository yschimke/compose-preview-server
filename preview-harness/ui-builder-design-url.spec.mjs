...
/** A design of its own for the one spec that needs two conversations on the same board. */
const twoThreadDesignId = "design-url-jetcaster-threads";

let server;
let seededRevision;
let threadId;
let firstOfTwoThreadId;
let secondOfTwoThreadId;
...
async function openDesign(page, selectors, onDesign = designId) {
  await page.goto(designPermalink(server, onDesign, selectors));
...
test.beforeAll(async () => {
  ...
  // A second design rather than a second thread on the first. Every thread card carries a Copy
  // link button with the same name, so a board with two conversations makes that locator ambiguous
  // for the specs that press it — and the fragment-navigation spec is the only one here that needs
  // two threads to move between.
  await seedJetcasterDesign(server, twoThreadDesignId);
  firstOfTwoThreadId = await seedCommentThread(server, twoThreadDesignId, {
    nodeId: "discover-grid",
    body: "Should this grid be two columns on a phone?",
  });
...
test("?node= selects the layer it names and opens Properties on it", async ({
  page,
}) => {
...
test("#thread= opens the Talk panel on the conversation it names", async ({
  page,
}) => {
...
test("a fragment-only navigation moves the panel to the thread it names", async ({
  page,
}) => {
  // The navigation this covers is the one the browser answers *without* reloading: following a
  // second thread link from inside the open design, which never re-enters the Wasm app. Driven
  // by assigning the hash, which is what such a link does.
  const opened = await openDesign(
    page,
    { thread: firstOfTwoThreadId },
    twoThreadDesignId,
  );
  expect(opened.threadId).toBe(firstOfTwoThreadId);
  await page.evaluate((hash) => {
    globalThis.location.hash = hash;
  }, `#thread=${secondOfTwoThreadId}`);
  await expect
    .poll(
      async () =>
...
test("#thread= combines with ?node= where the thread is pinned to a layer", async ({
  ...
  const selectors = await openDesign(page, {
    node: "discover-grid",
    thread: threadId,
  });
  expect(selectors).toMatchObject({
...
test("?revision= shows that revision read-only, with a way back to latest", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "Go to latest revision" }),
  ).toBeVisible();
});

test("an unknown ?revision= opens the latest design and says so", async ({
  page,
}) => {
...
test("a selected layer offers Copy link in its own menu", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: "Copy link to this layer" }),
  ).toBeVisible();
});

test("Copy link on a thread copies the canonical URL, with no token on it", async ({
  page,
}) => {
