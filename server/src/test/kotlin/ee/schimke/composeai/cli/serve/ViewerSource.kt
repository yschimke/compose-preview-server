 ...
 *
 * Reading the source is safe because the two cannot drift: `npm run verify` in `serve-web` rebuilds
 * the bundle and fails if the committed bytes differ from what this file produces, and CI runs it
 * on every PR.
 *
 ...
 */
internal fun viewerSource(): String = File(repoRoot(), "serve-web/src/viewer.ts").readText()
